<script lang="ts">
	...
	import { isCapacitor, loadDailySuggestions, loadPantryNames } from '$lib/stores/data';
	...
	onMount(async () => {
		if (isCapacitor()) {
			const [suggestions, pantryNms] = await Promise.all([
...
<div class="max-w-lg mx-auto px-4 pt-6 pb-16">
	<header class="mb-6">
		...
		<p class="text-warm-500 text-sm mt-1">{formatDate(localData.date)}</p>
	</header>
