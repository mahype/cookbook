<script lang="ts">
	...
	function switchTab(value: string) {
		...
		if (value !== 'einkaufsliste') {
			params.set('tab', value);
		}
		const query = params.toString();
		history.replaceState({}, '', '/lebensmittel' + (query ? '?' + query : ''));
	}
	...
	function handlePantryKeydown(e: KeyboardEvent) {
...
<div class="max-w-lg mx-auto px-4 pt-6">
	<header class="mb-4">
		<h1 class="text-2xl font-bold text-warm-900">Lebensmittel</h1>
	</header>

	<TabToggle tabs={tabOptions} active={activeTab} onChange={switchTab} />

	{#if activeTab === 'einkaufsliste'}
		<!-- Einkaufsliste tab -->
		{#if items.length === 0}
		...
		{:else}
			...
			{#each groupedUnchecked() as [storeName, storeItems] (storeName)}
				<div class="mb-4">
					<h2 class="text-sm font-semibold text-warm-700 mb-2">
						{storeEmojis[storeName] || '▸'} {storeName}
					</h2>
	...
	{:else}
		...
		{#if pantryError}
			<p class="text-sm text-red-600 mb-4 bg-red-50 px-4 py-2 rounded-xl">{pantryError}</p>
		{/if}
		...
		<p class="text-xs text-warm-400 text-center mt-4">{pantryItems.length} Artikel im Vorrat</p>
	{/if}
</div>
